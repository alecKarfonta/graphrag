...
class TwoStageFilter:
    ...
    def _assess_chunk_quality(self, chunk: RetrievedChunk, query: str) -> float:
        """
        Assess the quality of a chunk based on multiple factors.
        ...
        length_score = self._assess_length_quality(content)
        
        # 2. Information density
        density_score = self._assess_information_density(content)
        
        # 3. Coherence (basic heuristics)
        coherence_score = self._assess_coherence(content)
        
        # 4. Keyword-based quality indicators
        keyword_score = self._assess_keyword_quality(content)
        
        # 5. Completeness (sentences, punctuation)
